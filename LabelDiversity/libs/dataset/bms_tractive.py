...
@register_dataset("bms_tractive")
class BMSTractiveDataset(data.Dataset):
    ...
    def load_df(self):
        ...
        if self.mode == 'train':
            ...
            if self.oversampling:
                self.annotation_df["intlabel"] = self.annotation_df["label"].apply(self.check_pos)
    ...
    def __getitem__(self, index):
        ...
        soft_labels_tensor = self.labelBMS(row.label)
        return features,  soft_labels_tensor
    
    def labelBMS(self, strlabel):
        """
        Args:
            strlabel (string): string containing one label per annotator

        Returns:
            torch.tensor: weights for each label (0.5 if 2 different labels, 1 otherwise)
        """
        soft_labels = torch.zeros(2)
        if ',' in strlabel: 
            labels = strlabel.split(",")
            for l in labels:
                soft_labels[int(l)] += 0.5
        else:
            soft_labels[int(strlabel)] = 1
        return soft_labels
