...
def _labels2tokenset(spans, strict_tag, str_spans):
    #MObyGaze-modifif ranges = [[str(t) + "_" + _tag(s.tag, strict_tag) for t in _spanrange(s, str_spans)] for s in spans]
    ranges = [[str(t) + "_" + _tag(s.tag, strict_tag) for t in range(s.startend[0], s.startend[1], 1)] for s in spans]
