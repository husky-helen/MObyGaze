...
class InterAnnotatorAgreement(DoDa):
    ...
    def create_from_experiment(cls, experiment, distance_fn=None):
        ...
        return cls(experiment.annodf, experiment.item_colname, experiment.uid_colname, experiment.label_colname, distance_fn, experiment.uiddict, golddict)

    def __init__(self, annodf, item_colname, uid_colname, label_colname, distance_fn, uiddict, golddict=None):
    ...
    def setup(self, subsample_expected_distances=True, parallel_calc=False, precomputed_observed_distances=None):
        ...
        if isinstance(precomputed_observed_distances, int) and precomputed_observed_distances == 1: #MObyGaze-modifif
            ##Compute same-item distances between the 2 annotators
            ##Compute dist between diff random draws
            same_item = np.empty([len(self.all_labels),len(self.all_labels)])
            same_item[:] = np.NaN
            rand_item = np.empty([len(self.all_labels),len(self.all_labels)])
            rand_item[:] = np.NaN
            for ind_a, item_a in enumerate(self.items):
                for ind_b, item_b in enumerate(self.items):
                    if ind_a != ind_b:
                        worker_name_a = [key for key, val in self.uiddict.items() if val == self.workers[ind_a]][0]
                        worker_name_b = [key for key, val in self.uiddict.items() if val == self.workers[ind_b]][0]
                        if item_a == item_b:
                            if ((worker_name_a == 'worker_2') and (worker_name_b == 'worker_1')) or \
                                ((worker_name_a == 'worker_1') and (worker_name_b == 'worker_2')):
                                same_item[ind_a,ind_b] = 1
                            elif ((worker_name_a == 'worker_2') and ('worker_1' in worker_name_b) and ('worker_1' != worker_name_b)) or \
                                ((worker_name_b == 'worker_2') and ('worker_1' in worker_name_a) and ('worker_1' != worker_name_a)) or \
                                ((worker_name_a == 'worker_1') and ('worker_2' in worker_name_b) and ('worker_2' != worker_name_b)) or \
                                ((worker_name_b == 'worker_1') and ('worker_2' in worker_name_a) and ('worker_2' != worker_name_a)):
                                rand_item[ind_a,ind_b] = 1
                            
            print("Calculating same-item diff random draws annot distances")
            rand_item_ij = halfwhere(np.where(~np.isnan(rand_item)))
            dist_rand_same_item = gdm(self.all_labels, self.distance_fn, label_ij=rand_item_ij)
            self.expected_distances = dist_rand_same_item[rand_item_ij]
        ...
        elif precomputed_observed_distances is not None:
...
def split_iaa_by_item(iaa, split_items):
    ...
    for items in split_items:
        ...
        mini_iaa = InterAnnotatorAgreement(mini_df, "item", "worker", "label", iaa.distance_fn, iaa.uiddict)
