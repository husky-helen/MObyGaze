...
#MObyGaze-modif
class NERExperiment_MObyGaze(DecompositionExperiment):
    def __init__(self, eval_fn=None, dist_fn=None, **kwargs):
        ...
        self.data_dir = "/PATH_TO_CSV_FOLDER/" # TO BE SET BY THE USER
        self.common_films_ids = ['tt0097576', 'tt0110912', 'tt0375679', 'tt0970416', 'tt1193138',\
                                  'tt1568346', 'tt1907668', 'tt0108160', 'tt0119822', 'tt0467406', 'tt1045658',\
                                      'tt1285016', 'tt1570728', 'tt2267998', 'tt0109830', 'tt0120338', 'tt0822832', 'tt1142988', 'tt1454029', 'tt1632708']
        self.concept = None
        self.loose_tag = None
        self.random_concept_same_cutting = None
        self.n_factor = 1
        #self.gold_dir = "../seqcrowd-acl17/task1/val/ground_truth/"

    def setup(self, concept=None, random_concept_same_cutting=None, n_factor=None):

        def load(data_dir, common_films_ids):
            workers = []
            items = []
            raw_labels = []
            if self.random_concept_same_cutting is not None:
                workers_rand = []
                items_rand = []
                raw_labels_rand = []
            film_annotfile = data_dir + 'mobygaze_dataframe.csv'
            df_annots = pd.read_csv(film_annotfile, sep=";")
            annotators = df_annots['annotator'].unique()
            for ind, ann in enumerate(annotators):
                if "annotator_2" in ann:
                    ind_a2 = ind
                elif "annotator_1" in ann:
                    ind_a1 = ind
            name_a = annotators[ind_a2]
            name_b = annotators[ind_a1]
            for film_id in common_films_ids:
                print(film_id)
                df_annA = df_annots[(df_annots['imdb_key'] == film_id) & (df_annots['annotator'] == name_a)].reset_index(drop=True)
                df_annB = df_annots[(df_annots['imdb_key'] == film_id) & (df_annots['annotator'] == name_b)].reset_index(drop=True)
                                
                workers.append('worker_2')
                items.append(film_id)
                raw_labelA = raw2ranges(df_annA)
                raw_labels.append(raw_labelA)

                workers.append('worker_1')
                items.append(film_id)
                raw_labelB = raw2ranges(df_annB)
                raw_labels.append(raw_labelB)
                
                if self.random_concept_same_cutting is not None:
                    for ind_rand in range(self.n_factor):
                        workers_rand.append('worker_2_' + str(ind_rand))
                        items_rand.append(film_id)
                        raw_labelA = raw2ranges(df_annA, flag_observed_expected = 1)
                        raw_labels_rand.append(raw_labelA)

                        workers_rand.append('worker_1_' + str(ind_rand))
                        items_rand.append(film_id)
                        raw_labelB = raw2ranges(df_annB, flag_observed_expected = 1)
                        raw_labels_rand.append(raw_labelB)

            anno_df = pd.DataFrame({"worker":workers, "item":items, "annotation":raw_labels})
            if self.random_concept_same_cutting is not None:
                rand_df = pd.DataFrame({"worker":workers_rand, "item":items_rand, "annotation":raw_labels_rand})    
                anno_df = pd.concat([anno_df,rand_df],ignore_index=True)

            return anno_df
            
 
        def raw2ranges(df_ann, flag_observed_expected = 0, offset_scene = 0):
            if len(df_ann)>0:
                ''' represent annotations as sequences of token index ranges (with tags) '''
                if flag_observed_expected == 0:
                    result = []
                    for clip_ind in df_ann.index:
                        clip_tag = df_ann['label'][clip_ind]
                        if self.concept is not None:
                            list_concepts = df_ann['concepts'][clip_ind]
                            if self.concept in list_concepts:
                                clip_tag = self.concept
                            else:
                                clip_tag = 'not' + self.concept
                        if self.loose_tag is not None:
                                clip_tag = ''
                        newspan = {"range":[df_ann['start_frame'][clip_ind]-offset_scene, df_ann['end_frame'][clip_ind]-offset_scene], "tag": clip_tag}
                        sr = SeqRange(newspan["range"], tag=newspan["tag"])
                        result.append(sr)
                elif self.random_concept_same_cutting is not None and self.concept is not None:
                    '''
                    concept_app_count = []
                    for clip_ind in df_ann.index:
                        if self.concept in df_ann['concepts'][clip_ind]:
                            concept_app_count.append(self.concept)
                    if len(df_ann.index) > 0:
                        prob_concept = len(concept_app_count)/len(df_ann.index)
                    else:
                        prob_concept = 0
                    print('prob_concept=',prob_concept)
                    return
                    '''
                    concepts_probs={'Type': 0.12, 'Look': 0.13, 'Body': 0.16, 'Posture': 0.14, 'Clothes': 0.18, 'Appearance': 0.06, 'Activities': 0.1, 'Exp': 0.1, 'Voice': 0.11, 'Speech': 0.27, 'Soundtrack':0.02, 'Narratology': 0.012}
                    for concept in concepts_probs:
                            if self.concept in concept:
                                break
                    if self.concept not in concept:
                        print('error in prob_concept')
                        return 0
                    prob_concept = concepts_probs.get(concept)
                    result = []
                    for clip_ind in df_ann.index:
                        clip_tag = self.concept if random.random() < prob_concept else 'not' + self.concept
                        newspan = {"range":[df_ann['start_frame'][clip_ind]-offset_scene, df_ann['end_frame'][clip_ind]-offset_scene], "tag": clip_tag}
                        sr = SeqRange(newspan["range"], tag=newspan["tag"])
                        result.append(sr)
                return result

        self.concept = concept #"Body"
        self.random_concept_same_cutting = random_concept_same_cutting #1
        self.n_factor = n_factor #5
        #self.loose_tag = 1
        if concept is not None:
            anno_df = load(self.data_dir, self.common_films_ids).sort_values("item")
        else:
            anno_df = load(self.data_dir, self.common_films_ids).sort_values("item")

        super().setup(anno_df, None, c_anno_label="annotation", c_gold_label=None)        
...
def test_NER(debug=True):
    ...
    return test_iaa(NERExperiment, dist_fns, n_splits=10, cache_item_errors=False)

def test_MObyGaze(debug=True):
    eval_fns = {}
    dist_fns = {}
    #eval_fns["strict range strict tag"] = lambda x, y: eval_f1(x, y, strict_range=True, strict_tag=True, str_spans=True)
    #eval_fns["strict range lenient tag"] = lambda x, y: eval_f1(x, y, strict_range=True, strict_tag=False, str_spans=True)
    ...
    ##MObyGaze-modif return test_experiment("NER", NERExperiment, eval_fns, dist_fns, debug=debug)
    ##MObyGaze-modif: comment below to run IAA on concepts
    #return test_iaa(NERExperiment_MObyGaze, dist_fns, n_splits=10, cache_item_errors=False)

    # run analysis over all concepts
    concept_list = ['Type', 'Look', 'Body', 'Pos', 'Cloth', 'App', 'Activ', 'Exp', 'Voi', 'Speech', 'Sound']
    random_concept_same_cutting = 1
    n_factor = 5
    for concept in concept_list:
        print(concept)
        test_iaa(NERExperiment_MObyGaze, dist_fns, n_splits=10, cache_item_errors=False, concept=concept, random_concept_same_cutting=random_concept_same_cutting, n_factor=n_factor)
    return 0
...
def test_iaa(experiment_cls, distance_fns, n_splits=10, cache_item_errors=False, concept=None, random_concept_same_cutting=None, n_factor=None):
    the_exp = experiment_cls()
    the_exp.setup(concept, random_concept_same_cutting, n_factor)
    #the_exp.describe_data()

    n_items = len(the_exp.annodf[the_exp.item_colname].unique())
    items_per_split = int(n_items / n_splits)
    items_to_split = [the_exp.itemdict[key] for key in the_exp.itemdict] #None

    for key in distance_fns: #MObyGaze-modif
        distance_fn = distance_fns[key]
        #MObyGaze-modif iaa = agreement.InterAnnotatorAgreement.create_from_experiment(the_exp, distance_fn=distance_fn)
        iaa = agreement.InterAnnotatorAgreement.create_from_experiment(the_exp, distance_fn=distance_fn)
        iaa.setup(precomputed_observed_distances=the_exp.random_concept_same_cutting)
        #MObyGaze-modif-----
        krip = iaa.get_krippendorff_alpha()
        ks = iaa.get_ks()
        sig = iaa.get_sigma(debug=False)
        print('\niaa.get_krippendorff_alpha(): ', krip)
        print('iaa.get_ks(): ', ks) 
        print('iaa.get_sigma(): ', sig)
        if the_exp.concept is not None:
            iaa.plot_distance_distributions(title = the_exp.concept + ', krip=' + str(round(krip,2)) + ', ks=' + str(round(ks,2)) + ', sigma=' + str(round(sig,2)))
        else:
            #iaa.plot_distance_distributions(title = "_".join(the_exp.common_films_ids) + ', krip=' + str(round(krip,2)) + ', ks=' + str(round(ks,2)) + ', sigma=' + str(round(sig,2)))
            iaa.plot_distance_distributions(title = "all_films_" + ', krip=' + str(round(krip,2)) + ', ks=' + str(round(ks,2)) + ', sigma=' + str(round(sig,2)))
        return
        items_to_split = [6] ##########TO REVERSE#########
        mini_iaas = agreement.split_iaa_by_item(iaa, split_items=items_to_split)
        for ind_mini, mini_iaa in enumerate(mini_iaas):
            krip = mini_iaa.get_krippendorff_alpha()
            ks = mini_iaa.get_ks()
            sig = mini_iaa.get_sigma(debug=False)
            print('\niaa.get_krippendorff_alpha(): ', krip)
            print('iaa.get_ks(): ', ks) 
            print('iaa.get_sigma(): ', sig)
            if the_exp.concept is not None:
                mini_iaa.plot_distance_distributions(title = str(ind_mini) + the_exp.concept + ', krip=' + str(round(krip,2)) + ', ks=' + str(round(ks,2)) + ', sigma=' + str(round(sig,2)))
            else:
                mini_iaa.plot_distance_distributions(title = str(ind_mini) + ', krip=' + str(round(krip,2)) + ', ks=' + str(round(ks,2)) + ', sigma=' + str(round(sig,2)))
