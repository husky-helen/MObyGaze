...
def remove_duplicate_annotations(ants, tol=1e-3):
    ...
    valid_events = []
    for event in ants:
        if "segment" not in event.keys():
            event["segment"] = event["segment_frames"]
...
def load_gt_seg_from_json(json_file, split=None, label='label_id', label_offset=0, val_folds=None, test_folds=None):
    # load json file
    ...
    for k, v in json_db.items():
        imdb_id = k.split("_")[0]
        # ORIGINAL CASE
        if val_folds == None and test_folds == None:
            # filter based on split
            if (split is not None) and v['subset'].lower() != split:
                continue


        # NEW CASE WITH THE FOLDS
        else:
            if "train" in split: 
                if imdb_id in val_folds or imdb_id in test_folds: continue
            elif 'validation' in split :
                if imdb_id not in val_folds: continue
            elif 'test' in split:
                if imdb_id not in test_folds: continue
...
class ANETdetection(object):
    ...
    def __init__(
        ...
        self.ground_truth = load_gt_seg_from_json(
            ant_file, split=self.split, label=label, label_offset=label_offset, val_folds=val_folds, test_folds=test_folds)
        ...
        # print("ACTIVITY INDEX : ", self.activity_index)
    ...
    def evaluate(self, preds, verbose=True):
        ...
        'MAP': {f'tIoU_{tiou:.3f}': round(tiou_mAP,3) for tiou, tiou_mAP in zip(self.tiou_thresholds, mAP)},
        'Recall': {
            f'tIoU_{tiou:.3f}': {f'Recall@{k}x': round(float(rec),3)
                                for k, rec in zip(self.top_k, mrec_idx)}
            for tiou, mrec_idx in zip(self.tiou_thresholds, mRecall)
        },
        'average_mAP': round(average_mAP,3)
        }
            
       
        # return the results
        return mAP, average_mAP, mRecall, results_logs
...
def compute_average_precision_detection(
    ...
    tp_cumsum = np.cumsum(tp, axis=1).astype(float) #removed np.float because its deprecated
